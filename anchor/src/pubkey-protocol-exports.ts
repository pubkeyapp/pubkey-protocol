 ...
 *
 * NB: The `PubKeyIdentityProvider` enums are strings, but the program args input expects pubKeyIdentityProviderArgs
 * which is an object  * that gets generated by the enum in anchor/programs/pubkey-protocol/src/state/provider.rs as Tuple.
 * Can be a bit of a foot gun as they're distinct. Spent a good amount of time trying to massage the Rust into a format
 * to serialize to a string but the AnchorSerialize decorators are rather opinionated and it started to look
 * smelly trying to get them to output "string" rather than `{ string: {} }` which is the standard format for the enums.
 */
